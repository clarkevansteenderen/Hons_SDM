...
🟢 Using your ``sp_data_thin`` data frame, create another data frame called ``species_native``, where you filter the original so that it only keeps records where the longitude is greater than 60 degrees East. This object will contain all records from the psyllid's native range in Asia.

```{r eval=FALSE}

# Filter GPS records from the native range only
...
🟢 Now do the same for the invaded range, where you call the data frame ``species_invaded``. Filter the data so that you keep all records where longitude is less than 60 degrees East:
...
We need to generate background GPS points where we are unlikely to record ACP. This is quite a debated topic in SDM work, as these pseudo-absence points are generated based on statistical approaches, rather than actual verified data. Nevertheless, a good approach is to select points from regions that are at least somewhat similar to presence areas in terms of climate type. Here, we'll use Köppen-Geiger climate data to select similar eco-regions to randomly sample points from.
...
pres_buffer = st_buffer(pres_sf_proj, dist = 300000)
...
terra::plot(bg_area$wc2.1_2.5m_bio_1)
points(backPoints.buffer$lon, 
       backPoints.buffer$lat, 
       pch = 21, 
       bg = 'black', 
       cex = 0.2)
points(sp_data_thin$lon, 
...
backPoints.buffer = readRDS("data/backPoints.buffer.rds")
...
points(backPoints.buffer$lon, 
       ...
       bg = 'black', 
       cex = 0.2)
points(presPoints$decimalLongitude, 
       presPoints$decimalLatitude, 
       pch = 21, 
       bg = 'red', 
       cex = 1)
...
Let's plot the PCA again, but now let's have a look at whether there is any difference between the climate in the native versus the invaded range.
